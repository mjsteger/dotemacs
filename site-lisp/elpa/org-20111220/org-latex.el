...
(defun org-export-as-pdf (arg &optional hidden ext-plist
  ...
  (let* ((wconfig (current-window-configuration))
    ...
    (message (format "pdftk %s cat 1-$(($(pdftk %s dump_data output | grep NumberOfPages | cut -d : -f 2) -1)) output output.pdf" pdffile pdffile))
    (shell-command (format "mv output.pdf %s " pdffile))
